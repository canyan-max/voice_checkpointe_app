...
#define STORAGE_FILESYSTEM_SERVICE_BLOCK_CYCLES           500

static uint8_t storage_filesystem_read_buffer[
    STORAGE_FILESYSTEM_SERVICE_CACHE_SIZE];
static uint8_t storage_filesystem_program_buffer[
    STORAGE_FILESYSTEM_SERVICE_CACHE_SIZE];
static uint8_t storage_filesystem_lookahead_buffer[
    STORAGE_FILESYSTEM_SERVICE_LOOKAHEAD_SIZE];
static uint8_t storage_filesystem_file_buffer[
    STORAGE_FILESYSTEM_SERVICE_CACHE_SIZE];
...
_Static_assert(BOARD_STORAGE_CAPACITY_BYTES ==
                   BSP_STORAGE_FLASH_CAPACITY_BYTES,
...
static storage_filesystem_service_ret_t storage_filesystem_service_ret_from_lfs(
    int ret)
{
    switch(ret)
    {
...
static int storage_filesystem_service_address_get(
    const struct lfs_config *p_config,
    lfs_block_t block,
    lfs_off_t offset,
    lfs_size_t size,
    uint32_t *p_address)
{
    if((NULL == p_config) || (NULL == p_address) ||
       (block >= p_config->block_count) ||
       (offset > p_config->block_size) ||
                                       ...
                                       lfs_size_t size)
{
    ...
    ret = storage_filesystem_service_address_get(p_config,
                                             block,
                                             offset,
                                             size,
                                             &address);
    ...
    return storage_filesystem_service_ret_to_lfs(
        bsp_storage_flash_read(address,
                               (uint8_t *)p_buffer,
                               size,
                                          ...
                                          lfs_size_t size)
{
    ...
    return storage_filesystem_service_ret_to_lfs(
        bsp_storage_flash_program(address,
                                        ...
                                        lfs_block_t block)
{
    ...
    return storage_filesystem_service_ret_to_lfs(
        bsp_storage_flash_sector_erase(address,
                                       storage_filesystem_timeout_ms));
}
...
static storage_filesystem_service_ret_t storage_filesystem_service_blank_check(
    uint8_t *p_is_blank)
{
    platform_err_t ret;
    uint32_t offset;
    ...
    *p_is_blank = 1U;
    for(offset = 0U;
        offset < STORAGE_FILESYSTEM_SERVICE_REGION_SIZE_BYTES;
        offset += STORAGE_FILESYSTEM_SERVICE_CACHE_SIZE)
    {
        ...
        for(index = 0U;
            index < STORAGE_FILESYSTEM_SERVICE_CACHE_SIZE;
            index++)
        {
    ...
    uint8_t *p_was_formatted)
{
    ...
    *p_was_formatted = 0U;
    ...
    if(LFS_ERR_OK != lfs_ret)
    {
        ...
        lfs_ret = lfs_format(&storage_filesystem,
                             &storage_filesystem_config);
        if(LFS_ERR_OK != lfs_ret)
        {
            return storage_filesystem_service_ret_from_lfs(lfs_ret);
        }
        lfs_ret = lfs_mount(&storage_filesystem,
                            &storage_filesystem_config);
        if(LFS_ERR_OK != lfs_ret)
        {
            return storage_filesystem_service_ret_from_lfs(lfs_ret);
    ...
    storage_filesystem_service_open_mode_t mode)
{
    ...
    ret = lfs_file_opencfg(&storage_filesystem,
                           &storage_filesystem_file,
                           p_path,
                           flags,
                           &storage_filesystem_file_config);
    if(LFS_ERR_OK == ret)
    {
    ...
    uint32_t *p_read_size)
{
    ...
    size = lfs_file_read(&storage_filesystem,
                         &storage_filesystem_file,
                         p_data,
                         data_size);
    if(size < 0)
    ...
    uint32_t *p_write_size)
{
    ...
    size = lfs_file_write(&storage_filesystem,
                          &storage_filesystem_file,
                          p_data,
                          data_size);
    if(size < 0)
